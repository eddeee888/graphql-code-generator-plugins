...
import { getNodePropertyMap, isNodeTypeUnresolved } from './getNodePropertyMap.js';
...
describe('isNodeTypeUnresolved', () => {
  it('returns true for a mapper aliasing a type from an unresolvable import', () => {
    const project = new Project();
    const sourceFile = project.createSourceFile(
      '/path/to/mappers.ts',
      `
    ...
    const node = sourceFile.getFirstDescendant(
      (descendant) =>
        Node.isTypeAliasDeclaration(descendant) &&
        descendant.getName() === 'FooMapper'
    );

    expect(isNodeTypeUnresolved({ node })).toBe(true);
  });

  it('returns false for a mapper that genuinely has no properties', () => {
    const project = new Project();
    const sourceFile = project.createSourceFile(
      '/path/to/mappers.ts',
  ...
  it('returns false for an explicit `any` mapper', () => {
    const project = new Project();
    const sourceFile = project.createSourceFile(
      '/path/to/mappers.ts',
