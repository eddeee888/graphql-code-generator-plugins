...
`,
    ...
    const schemaType = typesSourceFile
    ...
    const schemaTypeRole = schemaType.find((t) => t.propertyName === 'role');
    const mapperTypeRole = mapperType.find((t) => t.propertyName === 'role');

    const schemaTypeCreatedAt = schemaType.find(
    ...
    expect(mapperTypeRole.type.isAssignableTo(schemaTypeRole.type)).toBe(true);
    expect(
      mapperTypeCreatedAt.type.isAssignableTo(schemaTypeCreatedAt.type)
    ).toBe(true);
  });
});
